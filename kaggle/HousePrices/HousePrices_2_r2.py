    ...
    with open("result2.csv", "w", newline="") as f:
        ...
        for i in range(k-1):
            ...
            for patternNo in range(keyNum):
                ...
                X_train_const = pd.DataFrame({"const" :[1]*X_train.shape[0]})
                X_train_const.index = X_train.index
                X_train_const = X_train_const.join(X_train)
                
                X_test_const = pd.DataFrame({"const" :[1]*X_test.shape[0]})
                X_test_const.index = X_test.index
                X_test_const = X_test_const.join(X_test)
                
                predict_train = result.predict(X_train_const)
